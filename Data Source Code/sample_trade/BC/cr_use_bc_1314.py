...
df_acct = sqlContext.read.parquet("/appdata/TU/TU_Official/Account/account_201[2-4]*_parq") \
  .select("TU_Consumer_ID","TU_Trade_ID","Joint_Account_ID","Primary_Indicator","ACCT_TYPE","PRODUCT_TYPE", \
          ...
          "PAYMT_AMT","ACTIVITY_PERIOD",col("TU_acct_num").alias("BIN"), \
  ...
  .where("PRODUCT_TYPE LIKE 'BC'").where("Run_date >= DATE('2012-12-01')")
...
df_acct.printSchema()
df_acct.where("Run_date >= DATE('2013-01-01')").write.parquet(path="df_acct_1314.parquet", mode="overwrite")
