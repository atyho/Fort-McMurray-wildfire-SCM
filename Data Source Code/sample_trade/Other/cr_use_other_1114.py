...
df_acct = sqlContext.read.parquet("/appdata/TU/TU_Official/Account/account_201[0-4]*_parq") \
  ...
  .where("NOT(ACCT_TYPE LIKE 'M') AND NOT(PRODUCT_TYPE RLIKE '(BC|ML|UT)')").where("Run_date >= DATE('2010-12-01')")
...
df_heloc_ls = sqlContext.read.parquet("../HELOC/df_heloc_ls.parquet")
...
df_acct = spark.sql("SELECT df_acct.*, \
                      CASE \
                        ...
                        WHEN PRODUCT_TYPE LIKE 'LC' THEN \
                          CASE \
                            WHEN df_heloc_ls.TU_Trade_ID IS NOT NULL THEN 'HELOC' \
                    ...
                    FROM df_acct \
                    LEFT JOIN df_heloc_ls \
                      ON df_acct.TU_Trade_ID = df_heloc_ls.TU_Trade_ID ") \
...
df_acct.printSchema()
df_acct.where("Run_date >= DATE('2011-01-01')").write.parquet(path="df_acct_1114.parquet", mode="overwrite")
