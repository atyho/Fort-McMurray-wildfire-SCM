...
#################################################
# For nonseverely damaged area in Fort McMurray #
#################################################

fsa_data <- fsa_data_nonsevere
...
#save.image(file = "FM_synth_qtr_CI_nsd.RData")

#####################################################
# Compute the confidence sets using function_SCM-CS #
#####################################################

load("FM_synth_qtr_CI_nsd.RData")
...
# Customized graph
FM_nonsevere.gap <- bind_cols(FM_nonsevere.data$tag$time.plot,
                              FM_nonsevere.data$Y1plot - (FM_nonsevere.data$Y0plot %*% FM_nonsevere.out$solution.w),
                              bounds)
colnames(FM_nonsevere.gap) <- c("date","gap","upper","lower")
write.csv(FM_nonsevere.gap,'FM_nonsevere_gap.csv')

plot_gap_nonsevere <- ggplot(data = FM_nonsevere.gap) + 
...
cairo_ps(filename = "plot_synth_gap_nonsevere.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
plot_gap_nonsevere
dev.off()
