...
df_ind = spark.sql("SELECT TU_Consumer_ID, Ref_date, \
                     ...
                     SUM(IF(terminal RLIKE 'WO', chargoff_refine, 0)) AS ml_chargoff, \
                     SUM(IF(terminal RLIKE 'WO', chargoff_new, 0)) AS ml_chargoff_new, \
                     SUM(IF(terminal NOT RLIKE 'WO' AND ml_insured = 1, CURRENT_BALANCE, 0)) AS ml_bal_ins, \
                     SUM(IF(MOP RLIKE '[4-5]' AND terminal NOT RLIKE 'WO' AND ml_insured = 1, CURRENT_BALANCE, 0)) AS ml_bal_arr_ins, \
                     SUM(IF(MOP RLIKE '[7-9]' AND terminal NOT RLIKE 'WO' AND ml_insured = 1, CURRENT_BALANCE, 0)) AS ml_bal_def_ins, \
...
df_ind = spark.sql("SELECT df_ind.*, \
                     df_ind_active.N_ml, df_ind_active.ml_lmt \
                   FROM df_ind \
