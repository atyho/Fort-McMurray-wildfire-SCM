...
df_ind = spark.sql("SELECT TU_Consumer_ID, Ref_date, \
                     SUM(IF(terminal NOT RLIKE 'WO', CURRENT_BALANCE, 0)) AS bc_bal, \
                     SUM(IF(MOP RLIKE '[4-5]' AND terminal NOT RLIKE 'WO', CURRENT_BALANCE, 0)) AS bc_bal_arr, \
                     SUM(IF(MOP RLIKE '[7-9]' AND terminal NOT RLIKE 'WO', CURRENT_BALANCE, 0)) AS bc_bal_def, \
                     SUM(IF(terminal RLIKE 'WO', chargoff_refine, 0)) AS bc_chargoff, \
                     SUM(IF(terminal RLIKE 'WO', chargoff_new, 0)) AS bc_chargoff_new \
                   FROM df_acct \
...
df_ind = spark.sql("SELECT df_ind.*, \
                     df_ind_active.N_bc, df_ind_active.bc_lmt \
                   FROM df_ind \
