...
df_acct = sqlContext.read.parquet("/appdata/TU/TU_Official/Account/account_201[0-8]*_parq") \
  ...
  .where("PRODUCT_TYPE LIKE 'LC'").where("Run_date >= DATE('2010-12-01')")
...
########################################################################
# HELOC: identify accounts by highest historical credit limit >= 50000 #
########################################################################

df_heloc_ls = spark.sql("SELECT DISTINCT TU_Trade_ID \
                        FROM df_acct \
                        GROUP BY TU_Trade_ID, NARRATIVE_CODE1, NARRATIVE_CODE2 \
                        HAVING \
                          MAX(cr_lmt)>=50000 \
                          OR FIRST(NARRATIVE_CODE1) RLIKE '^(SB)$' \
                          OR FIRST(NARRATIVE_CODE2) RLIKE '^(SB)$' ")

df_heloc_ls.createOrReplaceTempView("df_heloc_ls")              
df_heloc_ls.write.parquet(path="df_heloc_ls.parquet", mode="overwrite")
df_heloc_ls = sqlContext.read.parquet("df_heloc_ls.parquet")
df_heloc_ls.createOrReplaceTempView("df_heloc_ls")
              
df_acct = spark.sql("SELECT df_acct.* FROM df_acct \
                    WHERE df_acct.TU_Trade_ID IN (SELECT TU_Trade_ID FROM df_heloc_ls) ")
