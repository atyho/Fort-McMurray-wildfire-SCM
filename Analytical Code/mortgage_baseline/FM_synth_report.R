...
cairo_ps(filename = "figures/plot_synth_path_severe.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
plot_path_severe
dev.off()
...
cairo_ps(filename = "figures/plot_synth_path_nonsevere.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
plot_path_nonsevere
dev.off()
...
ML <- data %>% filter(treated==1 
  ...
  mutate(ml_ins_arr_rt = 100*(ml_bal_arr_ins_tot)/ml_bal_ins_tot,
         ml_arr_rt = 100*(ml_bal_arr_tot)/ml_bal_tot )

# Plots for mortgage arrears rate
plot_ML_severe <- ggplot() + 
  geom_line(data=ML%>%filter(treated==1 & FM_damage==1), 
            aes(y=ml_ins_arr_rt, x=date, color="ML_ins", linetype="ML_ins"), linewidth=0.75) +
  geom_line(data=ML%>%filter(treated==1 & FM_damage==1), 
            aes(y=ml_arr_rt, x=date, color="ML_all", linetype="ML_all"), linewidth=0.75) +
  geom_vline(xintercept=as.Date("2016-06-01", format="%Y-%m-%d"), color="black",linetype="solid") +
  labs(x=NULL, y=NULL) + coord_cartesian(ylim=c(0,2.0)) +
  scale_x_date(date_breaks="1 year", date_labels="%Y") +
  scale_y_continuous(breaks = seq(0.0, 2.0, by = 0.25)) +
  scale_colour_manual(name = "",
...
cairo_ps(filename = "figures/plot_path_severe.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
...
cairo_ps(filename = "figures/plot_path_nonsevere.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
...
plot_ML_stock_severe <- ggplot() + 
  geom_line(data=ML_severe_norm %>% filter(treated==1 & FM_damage==1), 
            aes(y=ml_bal_ins_norm, x=date, color="ML_ins_tot", linetype="ML_ins_tot"), linewidth=0.75) +
  geom_line(data=ML_severe_norm %>% filter(treated==1 & FM_damage==1), 
            aes(y=ml_bal_arr_ins_norm, x=date, color="ML_ins_arr", linetype="ML_ins_arr"), linewidth=0.75) +
  geom_vline(xintercept=as.Date("2016-06-01", format="%Y-%m-%d"), color="black",linetype="solid") +
  labs(x=NULL, y=NULL) + coord_cartesian(ylim=c(0.0,2.0)) +
  scale_x_date(date_breaks="1 year", date_labels="%Y") +
  scale_y_continuous(breaks = seq(0.0, 2.0, by = 0.25)) +
  scale_colour_manual(name = "",
...
cairo_ps(filename = "figures/plot_stock_severe.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
...
cairo_ps(filename = "figures/plot_stock_nonsevere.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
