...
#save.image(file = "FM_synth_CI_nonsevere.RData")

#####################################################
# Compute the confidence sets using function_SCM-CS #
#####################################################

load("FM_synth_CI_nonsevere.RData")
...
if(length(error_set) != 0){
  Ymat <- Ymat[, -error_set]
  weightsmat <- weightsmat[, -error_set]
...
# Customized graph
FM_nonsevere.gap <- bind_cols(date = FM_nonsevere.data$tag$time.plot, 
                              gap = ( FM_nonsevere.data$Y1plot - (FM_nonsevere.data$Y0plot %*% FM_nonsevere.out$solution.w) )[,1],
...
plot_gap_nonsevere <- ggplot(data = FM_nonsevere.gap) + 
  ...
  labs(x=NULL, y=NULL) + coord_cartesian(ylim=c(-2000, 2000)) +
...
cairo_ps(filename = "figures/plot_BCbal_gap_nonsevere.eps", width=3, height=3, pointsize = 12, fallback_resolution = 300)
plot_gap_nonsevere
dev.off()
